...
class EKF:
    def __init__(
        ...
        self._Fx: NDArray[float64] = np.eye(3, (3 + 3 * num_landmarks))
        self._I: NDArray[float64] = np.identity(self._Fx.shape[1])
    ...
    def predict(
        ...
        theta: float64 = theta_0 * self.timestep

        new_state_vector: NDArray[float64] = state_vector + np.matmul(
            self._Fx.transpose(), np.array([[x, y, theta]]).transpose()
        )
        print(f"Predicted motion: (x,y,theta):\n{new_state_vector}")
        ...
        shape_Fx = self._Fx.shape[0]
        g_t = np.zeros((shape_Fx, shape_Fx))
        g_t[:, 2] = np.array([g_x, g_y, 0.0]).transpose()

        G_t: NDArray[float64] = self._I + np.matmul(
            np.matmul(self._Fx.transpose(), g_t), self._Fx
        )

        print(f"Jacobian motion model:\n{G_t}")

        Sigma_t: NDArray[float64] = np.matmul(
            G_t, sigma_matrix, G_t.transpose()
        ) + np.matmul(np.matmul(self._Fx.transpose(), self._Rt), self._Fx)

        for i in measurement_vector:
            print(f"Correcting: {i}")

        print(f"State covariance matrix:\n{Sigma_t}")
...
#     def predict2(self, l_velocity=None, r_velocity=None):
