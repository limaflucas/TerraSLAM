...
class TrajectoryGraph:
    def __init__(self, event) -> None:
        ...
        (self.line_gt,) = self.ax.plot(
            [], [], "g-", label="Ground Truth (GPS)", linewidth=2
        )
        (self.line_prediction,) = self.ax.plot(
            [], [], "k--", label="EKF Prediction", alpha=0.6
        )
        (self.line_correction,) = self.ax.plot(
            [], [], "r-", label="EKF Correction", linewidth=1.5
        )

        self.ax.set_title("SLAM Trajectory Analysis")
...
class PoseEvolutionGraph:
    def __init__(self, event) -> None:
        plt.ion()
        self.fig, (self.ax_pose, self.ax_heading) = plt.subplots(
    ...
    def append_data_heading(self, compass_x, compass_y, ekf_theta):
        compass_theta = np.arctan2(compass_x, compass_y)
        heading = self.get_angle_diff(compass_theta, ekf_theta)
